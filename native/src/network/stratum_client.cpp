...
std::vector<std::uint8_t> hex_to_bytes(const std::string& hex) {
    ...
    for (std::size_t i = 0; i < out.size(); ++i) {
        out[i] = static_cast<std::uint8_t>(std::stoul(hex.substr(i * 2U, 2U), nullptr, 16));
    }
    return out;
}
...
std::array<std::uint8_t, 32> sha256(const std::vector<std::uint8_t>& input) {
    ...
    for (std::size_t i = 0; i < 8; ++i) {
        for (std::size_t j = 0; j < 4; ++j) out[i * 4 + j] = static_cast<std::uint8_t>(h[i] >> (24U - j * 8U));
    }
    return out;
}
...
std::string error_text(const json& error) {
    if (error.is_null()) return "unspecified pool response";
    if (error.is_string()) return error.get<std::string>();
    if (error.is_array() && error.size() > 1U && error[1].is_string()) return error[1].get<std::string>();
    return error.dump();
}

bool response_success(const json& message) {
    const auto result = message.find("result");
    if (result != message.end() && result->is_boolean()) return result->get<bool>();
...
public:
    ...
    void handle_response(const json& message, int id) {
        if (id == 1) {
            if (!response_success(message) || !message.contains("result") || !message["result"].is_array()) {
                throw std::runtime_error("pool subscription rejected: " + error_text(message.value("error", json(nullptr))));
            }
            const auto& result = message["result"];
            if (result.size() < 3U) throw std::runtime_error("short subscription response");
            extranonce1_ = result[1].get<std::string>();
            extranonce2_size_ = result[2].get<std::size_t>();
            log("Stratum subscribed: extranonce1=" + extranonce1_);
            return;
        }
        if (id == 2) {
            if (!response_success(message)) {
                throw std::runtime_error("pool authorization rejected: " + error_text(message.value("error", json(nullptr))));
            }
    ...
    void handle_notification(const json& message) {
        ...
        if (method == "mining.set_difficulty") {
            ...
            job.merkle_branch = p[4].get<std::vector<std::string>>();
            job.version = p[5].get<std::string>();
            job.nbits = p[6].get<std::string>();
            job.ntime = p[7].get<std::string>();
            if (p[8].is_boolean()) job.clean_jobs = p[8].get<bool>();
            else if (p[8].is_number_integer()) job.clean_jobs = p[8].get<int>() != 0;
            else job.clean_jobs = false;
            ...
            const auto& p = message.at("params");
            extranonce1_ = p.at(0).get<std::string>();
            extranonce2_size_ = p.at(1).get<std::size_t>();
        }
    }
...
Endpoint parse_endpoint(const std::string& url) {
    ...
    constexpr auto tcp = "stratum+tcp://";
    constexpr auto ssl = "stratum+ssl://";
    constexpr auto tcps = "stratum+tcps://";
    ...
    if (colon == std::string::npos) throw std::invalid_argument("pool URL must include port");
    endpoint.host = value.substr(0,colon);
    endpoint.port = static_cast<std::uint16_t>(std::stoul(value.substr(colon + 1U)));
...
MiningJob build_mining_job(const Job& job, const std::string& extranonce2) {
    ...
    for (const auto& branch_hex : job.merkle_branch) {
        ...
        joined.reserve(64);
        joined.insert(joined.end(), merkle.begin(), merkle.end());
        joined.insert(joined.end(), branch.begin(), branch.end());
        merkle = sha256d(joined);
    }
    MiningJob result;
    result.job_id = job.job_id;
    result.version = parse_hex_u32(job.version);
    result.previous_hash = parse_prevhash_words(job.prevhash);
    std::copy(merkle.begin(), merkle.end(), result.merkle_root.begin());
    result.ntime = parse_hex_u32(job.ntime);
    result.bits = parse_hex_u32(job.nbits);
    return result;
}
