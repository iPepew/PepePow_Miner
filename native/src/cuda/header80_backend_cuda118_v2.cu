...
// Job-wide device state. Each GPU owns its own CUDA address space, matching the
// existing per-device backend model used by the miner.
__device__ double d_matrix[64][64];
__device__ std::uint32_t d_first_cv[8];
...
__device__ __forceinline__ std::uint32_t bswap32(std::uint32_t x){
...
__device__ void first_block_cv(const std::uint8_t* in,std::uint32_t cv[8]){
    std::uint32_t iv[8],b[16],c[16];
    #pragma unroll
    for(int i=0;i<8;++i)iv[i]=kIv[i];
    #pragma unroll
    for(int i=0;i<16;++i)b[i]=le32(in+4*i);
    compress(iv,b,64,kChunkStart,c);
    #pragma unroll
    for(int i=0;i<8;++i)cv[i]=c[i];
}

// Compute an 80-byte BLAKE3 hash when the first 64-byte chunk CV is already
...
// One invocation per new job: cache the first BLAKE3 CV and generate the
...
__device__ __forceinline__ std::uint8_t nibble(const std::uint8_t first[32],int j){
...
void Header80CudaBackend::prepare_job(const Header80& header){
    ...
    cuda_check(cudaGetLastError(),"prepare_job_kernel launch");
...
std::string_view Header80CudaBackend::name() const noexcept{return "cuda-header80-kernel-v2-streamed-precv-cuda118";}
...
std::optional<ShareCandidate> Header80CudaBackend::search(const MiningJob& job,SearchRange range,const Hash256& target){
    ...
    // Keep baseline launch geometry for the first Kernel-v2 A/B test. Geometry
