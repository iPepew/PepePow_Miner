...
// Two independent 64-thread producer/service pairs inside one Foztor-sized block:
// warp0 owners <-> warp1 services, warp2 owners <-> warp3 services.
// Each block advances 64 independent nonce owners while synchronization is
// restricted to the corresponding pair via Volta named barriers.
__global__ __launch_bounds__(128, 1)
void warp_pair128_namedbarrier_probe(
    const double* __restrict__ input,
    const std::uint32_t* __restrict__ selector,
    double* __restrict__ output,
    std::size_t owner_count) {
    __shared__ PairScratch scratch[2];
    ...
    const std::size_t owner_index = static_cast<std::size_t>(blockIdx.x) * 64U + owner_local;
