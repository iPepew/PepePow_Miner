...
std::array<std::uint32_t, 256> validation_nonces() {
    ...
    for (std::size_t i = 6; i < values.size(); ++i) {
        ...
        values[i] = static_cast<std::uint32_t>(
            0x243f6a88U ^ (n * 0x9e3779b9U) ^ (n << 17U) ^ (n >> 7U));
    }
    return values;
}
...
int main(int argc, char** argv) {
    try {
        std::string engine = "magic";
        unsigned int threads = 256U;
        unsigned int blocks_per_sm = 2U;
        unsigned int fast_rsqrt = 0U;
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            else if (arg == "--fast-rsqrt" && i + 1 < argc) fast_rsqrt = parse_u32(argv[++i], "fast-rsqrt");
            else if (arg == "--count" && i + 1 < argc) count = parse_u64(argv[++i], "count");
            else if (arg == "--rounds" && i + 1 < argc) rounds = parse_u32(argv[++i], "rounds");
            else if (arg == "--min-quality" && i + 1 < argc) minimum_quality = std::stod(argv[++i]);
            else if (arg == "--help") {
                std::cout
                    << "Usage: pepepow-v120-autotune --engine exact|magic --threads N "
                    << "[--blocks-per-sm N] [--fast-rsqrt 0|1] [--count N] [--rounds N] "
                    << "[--min-quality 0..1]\n";
                return 0;
            } else {
        ...
        if (engine != "exact" && engine != "magic") {
            throw std::invalid_argument("engine must be exact or magic");
        }
        if (threads < 96U || threads > 768U || (threads % 32U) != 0U) {
            throw std::invalid_argument("threads must be a multiple of 32 in [96,768]");
        }
        if (engine == "magic" && threads > 256U) {
            throw std::invalid_argument("magic engine supports at most 256 threads/block");
        }
        if (blocks_per_sm < 1U || blocks_per_sm > 16U) {
            throw std::invalid_argument("blocks-per-sm must be in [1,16]");
        }
        if (fast_rsqrt > 1U) throw std::invalid_argument("fast-rsqrt must be 0 or 1");
        ...
        if (!(minimum_quality >= 0.0 && minimum_quality <= 1.0)) {
            throw std::invalid_argument("min-quality must be in [0,1]");
        }

        if (::setenv("PEPEPOW_V120_ENGINE", engine.c_str(), 1) != 0) {
            throw std::runtime_error("setenv failed for PEPEPOW_V120_ENGINE");
        }
        set_env_u32("PEPEPOW_V120_BLOCKS_PER_SM", blocks_per_sm);
        set_env_u32("PEPEPOW_V120_FAST_RSQRT", fast_rsqrt);
        ...
        if (device.compute_major != 7 || device.compute_minor != 0) {
            throw std::runtime_error("v1.2.0 V100 autotune requires Volta sm_70");
        }
        ...
        for (const auto nonce : nonces) {
            ...
            if (gpu.has_value() && gpu->nonce == nonce && gpu->hash == cpu_hash) ++matches;
        }

        const double quality = static_cast<double>(matches) / static_cast<double>(nonces.size());
        const bool quality_ok = engine == "exact" ? matches == nonces.size() : quality >= minimum_quality;
        if (!quality_ok) {
            std::cout << std::fixed << std::setprecision(6)
                      ...
                      << " valid=0 quality=" << quality
                      << " matches=" << matches << '/' << nonces.size()
                      << " hps=0 effective_hps=0 reason=quality_gate\n";
            return 3;
        }
        ...
        const double effective = median * quality;
        ...
        std::cout << std::fixed << std::setprecision(0)
                  ...
                  << " matches=" << matches
                  << " quality=" << std::setprecision(6) << quality
                  << std::setprecision(0)
                  << " hps=" << median
                  << " mean_hps=" << mean
                  << " effective_hps=" << effective
                  << " mhs=" << std::setprecision(3) << median / 1000000.0
                  << " effective_mhs=" << effective / 1000000.0
                  << " device=\"" << device.name << "\"\n";
        return 0;
