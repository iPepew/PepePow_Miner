...
if [[ -r "${self_dir}/pepepow-debug.log" ]]; then
  ...
  grep -E 'target_source=nbits_div_difficulty|target_be=' \
    "${self_dir}/pepepow-debug.log" > "${work}/target-boundary-evidence.txt" 2>/dev/null || true
fi
