...
for f in VERSION h-manifest.conf h-config.sh h-run.sh h-stats.sh config.txt setup.txt run.txt pepepow-debug.log; do
...
if command -v sha256sum >/dev/null 2>&1; then
  sha256sum "${self_dir}/pepepowminer" "${self_dir}"/*.sh "${self_dir}/VERSION" > "${work}/sha256.txt" 2>/dev/null || true
fi
