...
PROFILE_ROOT="${ROOT_DIR}/build-profiles-v054"
LINK_BUILD_DIR="${ROOT_DIR}/build-rtx3080-v054"
...
JOBS="${JOBS:-$(nproc)}"
DEPS_DIR="${ROOT_DIR}/.deps-v054"
PREPARE_SCRIPT="${ROOT_DIR}/hiveos/prepare-v054-source.py"
EXPECTED_EDITION="PepeW Lookup Split Edition"
...
VERSION="$(head -n1 "${ROOT_DIR}/VERSION" | tr -d '[:space:]')"
[[ "${VERSION}" == "0.5.4-PR" ]] || { echo "Expected VERSION=0.5.4-PR, found ${VERSION}" >&2; exit 1; }
PACKAGE_NAME="pepepowminer-v${VERSION}"
PACKAGE_DIR="${ROOT_DIR}/dist/${PACKAGE_NAME}"
ARCHIVE_PATH="${ROOT_DIR}/dist/${PACKAGE_NAME}-hiveos.tar.gz"

for marker in PEPEPOW_CUDA_SCALED_NIBBLE_TABLE PEPEPOW_CUDA_ASSUME_FINITE kScaledNibbleElements; do
  grep -Fq "${marker}" "${ROOT_DIR}/native/src/cuda/header80_backend_v054.cu" || {
    echo "Missing v0.5.4 CUDA marker: ${marker}" >&2; exit 1;
  }
done
grep -Fq 'PASS: 16384 lookup-split CPU/CUDA samples match' \
  "${ROOT_DIR}/native/tests/cuda_header80_validation.cpp" || {
  echo "16384-sample validation gate missing" >&2; exit 1;
}
...
build_profile() {
  local name="$1" threads="$2" min_blocks="$3" split="$4" lookup="$5" finite="$6" max_regs="$7"
  local dir="${PROFILE_ROOT}/${name}"
  local build_log="${dir}.build.log"
  rm -rf "${dir}" "${build_log}"
  echo "== PROFILE ${name}: threads=${threads} min_blocks=${min_blocks} split=${split} lookup=${lookup} finite=${finite} max_regs=${max_regs:-auto} =="
  ...
  local cmake_args=(
    ...
    -DPEPEPOW_CUDA_SCALED_NIBBLE_TABLE="${lookup}"
    -DPEPEPOW_CUDA_ASSUME_FINITE="${finite}"
  ...
  cmake --build "${dir}" --parallel "${JOBS}" \
    --target pepepow_core_tests pepepow_cuda_tests pepepow_cuda_header80_validation \
             pepepow_header80_benchmark pepepowminer 2>&1 | tee "${build_log}"

  if ! ctest --test-dir "${dir}" --output-on-failure; then
    printf 'name=%s\nvalid=0\nreason=ctest_failed\nthreads=%s\nmin_blocks=%s\nsplit=%s\nlookup=%s\nfinite=%s\nmax_regs=%s\n' \
      "${name}" "${threads}" "${min_blocks}" "${split}" "${lookup}" "${finite}" "${max_regs:-auto}" > "${dir}/profile.meta"
  ...
  pow_regs="${pow_regs:-0}"; mix_regs="${mix_regs:-0}"

  printf '%s\n' "${selected_hps}" > "${dir}/profile.hps"
  printf 'name=%s\nvalid=1\nthreads=%s\nmin_blocks=%s\nsplit=%s\nlookup=%s\nfinite=%s\nmax_regs=%s\nmax_registers=%s\npow_registers=%s\nmix_registers=%s\nmax_spill_bytes=%s\nbenchmark_runs=%s\nbenchmark_hps=%s\nmedian_hps=%s\n' \
    "${name}" "${threads}" "${min_blocks}" "${split}" "${lookup}" "${finite}" "${max_regs:-auto}" \
    "${regs}" "${pow_regs}" "${mix_regs}" "${spills}" "${BENCH_RUNS}" "${speeds[*]}" "${selected_hps}" > "${dir}/profile.meta"
  printf 'PROFILE_RESULT name=%s threads=%s min_blocks=%s split=%s lookup=%s finite=%s max_regs=%s registers=%s pow_regs=%s mix_regs=%s spills=%s median_hps=%s\n' \
    "${name}" "${threads}" "${min_blocks}" "${split}" "${lookup}" "${finite}" "${max_regs:-auto}" \
    "${regs}" "${pow_regs}" "${mix_regs}" "${spills}" "${selected_hps}"
}

profiles=()
# Feature isolation at the proven 64-thread geometry.
build_profile base-mono64          64 1 OFF OFF OFF 80; profiles+=(base-mono64)
build_profile finite-mono64        64 1 OFF OFF ON  80; profiles+=(finite-mono64)
build_profile lookup-mono64        64 1 OFF ON  OFF 80; profiles+=(lookup-mono64)
build_profile lookup-finite-mono64 64 1 OFF ON  ON  80; profiles+=(lookup-finite-mono64)

build_profile base-split64          64 1 ON OFF OFF ""; profiles+=(base-split64)
build_profile finite-split64        64 1 ON OFF ON  ""; profiles+=(finite-split64)
build_profile lookup-split64        64 1 ON ON  OFF ""; profiles+=(lookup-split64)
build_profile lookup-finite-split64 64 1 ON ON  ON  ""; profiles+=(lookup-finite-split64)

stage_best=""; stage_hps=0
for profile in "${profiles[@]}"; do
  [[ -s "${PROFILE_ROOT}/${profile}/profile.hps" ]] || continue
  hps="$(cat "${PROFILE_ROOT}/${profile}/profile.hps")"
  if (( hps > stage_hps )); then stage_hps="${hps}"; stage_best="${profile}"; fi
done
[[ -n "${stage_best}" ]] || { echo "No valid feature profile" >&2; exit 1; }
# shellcheck disable=SC1090
source <(sed -n -E 's/^(split|lookup|finite)=(.*)$/BEST_\1=\2/p' "${PROFILE_ROOT}/${stage_best}/profile.meta")
: "${BEST_split:?}" "${BEST_lookup:?}" "${BEST_finite:?}"
printf 'FEATURES_SELECTED profile=%s hps=%s split=%s lookup=%s finite=%s\n' \
  "${stage_best}" "${stage_hps}" "${BEST_split}" "${BEST_lookup}" "${BEST_finite}"

# Geometry and launch-bound sweep for the best measured feature set.
build_profile best-64-auto  64 1 "${BEST_split}" "${BEST_lookup}" "${BEST_finite}" ""; profiles+=(best-64-auto)
build_profile best-64-b2    64 2 "${BEST_split}" "${BEST_lookup}" "${BEST_finite}" ""; profiles+=(best-64-b2)
build_profile best-96       96 1 "${BEST_split}" "${BEST_lookup}" "${BEST_finite}" ""; profiles+=(best-96)
build_profile best-128     128 1 "${BEST_split}" "${BEST_lookup}" "${BEST_finite}" ""; profiles+=(best-128)
build_profile best-192     192 1 "${BEST_split}" "${BEST_lookup}" "${BEST_finite}" ""; profiles+=(best-192)
build_profile best-256     256 1 "${BEST_split}" "${BEST_lookup}" "${BEST_finite}" ""; profiles+=(best-256)
build_profile best-320     320 1 "${BEST_split}" "${BEST_lookup}" "${BEST_finite}" ""; profiles+=(best-320)
...
[[ -s "${PROFILE_ROOT}/base-mono64/profile.hps" ]] || { echo "Consensus baseline failed; refusing to package" >&2; exit 1; }
baseline_hps="$(cat "${PROFILE_ROOT}/base-mono64/profile.hps")"
...
for tool in collect-profiler-pack-v4.sh collect-v053-profiler.sh collect-v053-deep-direct.sh collect-v054-profiler.sh; do
...

{
  ...
  echo "baseline_profile=base-mono64"
  ...
  echo "source_backend=header80_backend_v054.cu"
  echo "profiler_evidence=short_scoreboard_30.71 active_threads_per_warp_8.10 achieved_occupancy_34.95"
  echo
  for profile in "${profiles[@]}"; do
    echo "--- ${profile} ---"
...
cat > "${PACKAGE_DIR}/README-v054.txt" <<EOF
PepeW Miner ${VERSION}
...
This prerelease is consensus-gated. Do not claim 2 MH/s unless both the
internal TARGET_2MH gate and a live pool validation exceed the target.
EOF
...
echo "PASS: PepeW Lookup Split Edition ${VERSION} package completed"
