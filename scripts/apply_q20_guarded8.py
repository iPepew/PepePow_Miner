...
new_loop = """            #pragma unroll 1
            for (int col=0;col<64;) {
                ...
                if (!cold && col+7<64) {
                    ...
                    const std::uint8_t b2=byte_at(first,col2>>1);
                    const std::uint32_t nib2=(col2&1)?(b2&0x0fU):(b2>>4U);
                    const std::uint64_t d2=static_cast<std::uint64_t>(static_cast<std::int64_t>(table[row*64+col2])*static_cast<std::int64_t>(nib2));
                    const int col3=col+3;
                    const std::uint8_t b3=byte_at(first,col3>>1);
                    const std::uint32_t nib3=(col3&1)?(b3&0x0fU):(b3>>4U);
                    const std::uint64_t d3=static_cast<std::uint64_t>(static_cast<std::int64_t>(table[row*64+col3])*static_cast<std::int64_t>(nib3));
                    const int col4=col+4;
                    const std::uint8_t b4=byte_at(first,col4>>1);
                    const std::uint32_t nib4=(col4&1)?(b4&0x0fU):(b4>>4U);
                    const std::uint64_t d4=static_cast<std::uint64_t>(static_cast<std::int64_t>(table[row*64+col4])*static_cast<std::int64_t>(nib4));
                    const int col5=col+5;
                    const std::uint8_t b5=byte_at(first,col5>>1);
                    const std::uint32_t nib5=(col5&1)?(b5&0x0fU):(b5>>4U);
                    const std::uint64_t d5=static_cast<std::uint64_t>(static_cast<std::int64_t>(table[row*64+col5])*static_cast<std::int64_t>(nib5));
                    const int col6=col+6;
                    const std::uint8_t b6=byte_at(first,col6>>1);
                    const std::uint32_t nib6=(col6&1)?(b6&0x0fU):(b6>>4U);
                    const std::uint64_t d6=static_cast<std::uint64_t>(static_cast<std::int64_t>(table[row*64+col6])*static_cast<std::int64_t>(nib6));
                    const int col7=col+7;
                    const std::uint8_t b7=byte_at(first,col7>>1);
                    const std::uint32_t nib7=(col7&1)?(b7&0x0fU):(b7>>4U);
                    const std::uint64_t d7=static_cast<std::uint64_t>(static_cast<std::int64_t>(table[row*64+col7])*static_cast<std::int64_t>(nib7));
                    const std::uint64_t rem0=qsum & kQPeriodMask;
                    const std::uint64_t room=kQPeriodMask-rem0;
                    const std::uint64_t dsum=d0+d1+d2+d3+d4+d5+d6+d7;
                    if (dsum<=room) {
                        qsum+=dsum;
                        col+=8;
                        continue;
