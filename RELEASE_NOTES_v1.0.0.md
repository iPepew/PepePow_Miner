...
В финальной branded-сборке v1.0.0 повторный release benchmark показал медиану `2.209014 MH/s`; бинарник имеет SHA256:
...
## Финальный HiveOS-пакет

Публичный пакет проходит отдельный privacy/sanitation gate. В него не включаются файлы, созданные во время работы установленного майнера: конфигурация кошелька, журналы Stratum, диагностические логи, PID-файлы, старые статусы и runtime-снимки.

Финальный пакет содержит только 13 необходимых файлов: бинарник, HiveOS-скрипты запуска и телеметрии, passive proxy, документы релиза и контрольные суммы. Внутренний `h-manifest.conf` имеет версию `1.0.0`, а права доступа и владельцы файлов нормализованы.
...
Сборщик релиза не изменяет частоты, Power Limit или прошивку GPU, не устанавливает пакет поверх текущего майнера и не запускает его автоматически. Сначала создаётся отдельный архив HiveOS с SHA256. Перед публикацией архив пересобирается из белого списка файлов и проверяется на отсутствие runtime-данных и адреса кошелька.
