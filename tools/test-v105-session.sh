...
STARTUP_TIMEOUT="${PEPEW_STARTUP_TIMEOUT:-120}"
...
[[ "$STARTUP_TIMEOUT" =~ ^[1-9][0-9]*$ ]] || STARTUP_TIMEOUT=120
...
finalize() {
  ...
  finalized=1
  trap - INT TERM EXIT
  local end_epoch elapsed xid_start_count=0 i logfile
  end_epoch="$(date +%s)"
  ...
  [[ -f "$DIR/config.txt" ]] && cp -a "$DIR/config.txt" "$session_dir/" 2>/dev/null || true

  for ((i=0; i<EXPECTED_GPU_COUNT; ++i)); do
    logfile="$LOG_ROOT/gpu${i}/pepew.log"
    [[ -f "$logfile" ]] && cp -a "$logfile" "$session_dir/logs/gpu${i}-pepew.log" 2>/dev/null || true
    for f in stratum-proxy.log proxy-console.log; do
      [[ -f "$LOG_ROOT/gpu${i}/$f" ]] && cp -a "$LOG_ROOT/gpu${i}/$f" "$session_dir/logs/gpu${i}-$f" 2>/dev/null || true
    done
...

  {
    ...
    for ((i=0; i<EXPECTED_GPU_COUNT; ++i)); do
      local l="$session_dir/logs/gpu${i}-pepew.log"
      [[ -f "$l" ]] || continue
      printf 'GPU%s ACCEPTED=%s REJECTED=%s ERROR=%s JOB=%s MINING=%s\n' "$i" "$(grep -ac '^\[ACCEPTED\]' "$l" 2>/dev/null || true)" "$(grep -ac '^\[REJECTED\]' "$l" 2>/dev/null || true)" "$(grep -ac '^\[ERROR\]' "$l" 2>/dev/null || true)" "$(grep -ac '^\[JOB\]' "$l" 2>/dev/null || true)" "$(grep -ac '^\[MINING\]' "$l" 2>/dev/null || true)"
    done
  } > "$summary"
...
trap 'interrupted=1; session_result=INTERRUPTED; finalize; exit 130' INT TERM
trap 'finalize' EXIT

echo '===== PepeW Miner v1.0.5 robust session capture ====='
...
echo '[CAPTURE] waiting for worker/proxy and positive hashrate...'
...
while :; do
  ...
  if (( ready_epoch == 0 && miner_count >= EXPECTED_GPU_COUNT && proxy_count >= EXPECTED_GPU_COUNT && ready_gpu_count >= EXPECTED_GPU_COUNT )); then
    ...
    process_loss_since=0
    echo "[CAPTURE] READY after ${elapsed}s; 10-minute mining timer starts now." | tee -a "$session_dir/session-events.txt"
  fi
