...
NAME="${PEPEW_NAME:-PepeW-Miner-v1.0.4-HiveOS}"
...
printf '%s\n' '===== PepeW Miner v1.0.4 live validation ====='
printf 'package=%s\nexpected_gpus=%s\n' "$DIR" "$EXPECTED_GPU_COUNT"

[[ -d "$DIR" ]] && pass "package directory" || fail "package directory missing"
for file in h-manifest.conf h-config.sh h-run.sh h-stats.sh pepepowminer stratum-replay-proxy.py; do
...
printf 'binary=%s\n' "$version"
grep -Fq '1.0.4' <<<"$version" && pass "binary version" || fail "binary version"
...
printf 'status_files=%s\n' "$status_count"

if [[ -s "$DIR/h-stats.sh" ]]; then
  ...
  grep -Fq '"ver":"1.0.4"' <<<"$telemetry" && pass "telemetry version" || fail "telemetry version"
fi

printf '%s\n' '----- active workers -----'
cat "$DIR/active-workers.env" 2>/dev/null || true

printf '%s\n' '----- last mining lines -----'
for ((i=0; i<EXPECTED_GPU_COUNT; ++i)); do
  log="$LOG_ROOT/gpu${i}/pepew.log"
  printf '%s\n' "[GPU $i]"
  grep -aE '\[MINING\]|\[ACCEPTED\]|\[REJECTED\]' "$log" 2>/dev/null | tail -n 8 || true
done

printf '%s\n' '----- GPU state -----'
nvidia-smi --query-gpu=index,pci.bus_id,name,temperature.gpu,fan.speed,power.draw --format=csv,noheader 2>/dev/null || true
...
if (( failures == 0 )); then
  echo 'PEPEW_V104_LIVE_GATE=PASS'
  exit 0
fi

echo "PEPEW_V104_LIVE_GATE=FAIL failures=${failures}"
exit 1
