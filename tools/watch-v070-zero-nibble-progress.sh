...
while true; do
  ...
  last_bench="$(grep -a '^BENCH_RUN ' "$LOG" 2>/dev/null | tail -n1)"
  echo 'ТЕКУЩИЙ BENCHMARK'
  echo '----------------------------------------------------------------------'
  if [[ -n "$last_bench" ]]; then
    echo "$last_bench"
  else
    echo 'Ожидание первого benchmark-прогона.'
  fi
  echo
  ...
  echo '----------------------------------------------------------------------'
  grep -a '^PROFILE_RESULT ' "$LOG" 2>/dev/null | tail -n3 || true
  echo
  echo "Журнал: $LOG"
