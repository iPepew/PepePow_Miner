...
void print_version() {
    std::cout << "PepePowMiner " << PEPEPOW_VERSION
              << " commit=" << PEPEPOW_GIT_COMMIT
              << " build=" << PEPEPOW_BUILD_TYPE << '\n';
}
...
void print_help() {
    ...
    std::cout
        ...
        << "      --diagnostic     Print target, header and submit diagnostics\n"
        << "      --list-gpu       List detected devices and exit\n"
        << "      --version        Print exact build identity\n"
...
class MiningWorker {
public:
    MiningWorker(pepepow::MiningBackend& backend, pepepow::stratum::Client& client, bool diagnostic)
        : backend_(backend), client_(client), diagnostic_(diagnostic),
    ...
    void run() {
        ...
        while (!stopped_.load()) {
            ...
            try {
                ...
                if (diagnostic_) {
                    auto diagnostic_job = mining_job;
                    diagnostic_job.nonce = 0;
                    const auto header = pepepow::build_header80(diagnostic_job);
                    std::cout << "TARGET_DIAG job=" << mining_job.job_id
                              << " target_be=" << bytes_hex(target)
                              << " target_rev=" << bytes_hex(std::span(target).subspan(0)) << '\n';
                    std::cout << "JOB_DIAG job=" << mining_job.job_id
                              << " header80_nonce0=" << bytes_hex(header)
                    ...
                    if (candidate.has_value()) {
                        ...
                        if (cpu_hash != candidate->hash) {
                            ...
                            std::cout << "Validated share candidate nonce=" << share.nonce
                                      ...
                                      << " hash=" << bytes_hex(cpu_hash) << '\n';
                            if (diagnostic_) {
                                std::cout << "SHARE_DIAG job=" << share.job_id
                                          << " nonce_wire=" << share.nonce
                                          << " nonce_u32=" << candidate->nonce
                                          << " ntime=" << share.ntime
                                          << " xnonce2=" << share.extranonce2
                                          << " header80=" << bytes_hex(header)
                                          << " hash_be=" << bytes_hex(cpu_hash)
                                          << " target_be=" << bytes_hex(target) << '\n';
                            }
...
int main(int argc, char** argv) {
    try {
        ...
        for (int index = 1; index < argc; ++index) {
            ...
            else if (argument == "--version") { print_version(); return 0; }
        ...
        print_version();
        std::cout << "BUILD_ID version=" << PEPEPOW_VERSION
                  << " commit=" << PEPEPOW_GIT_COMMIT
        ...
        config.agent = std::string("PepePowMiner/") + PEPEPOW_VERSION;

        pepepow::stratum::Client client(std::move(config));
        MiningWorker worker(*backend, client, diagnostic);
