...
# Durable launcher for HiveOS web-shell sessions. The actual validation process
# is detached with nohup, so closing/reconnecting shell.hiveos.farm does not
# terminate the test. Progress is written to a stable log path.

SCRIPT_URL="${PEPEW_TEST_SCRIPT_URL:-https://raw.githubusercontent.com/iPepew/PepePow_Miner/agent/v100-pro-hotloop/lab/pepew-hardware-test.sh}"
...
REPORT_ROOT="${PEPEW_REPORT_DIR:-/tmp/pepew-v100-detached}"

ASSET_URL="${PEPEW_ASSET_URL:-https://github.com/iPepew/PepePow_Miner/releases/download/hiveos-v100-test/PepeW-Miner-HiveOS.tar.gz}"
...
# The wrapper records the final report path even if the web terminal disappears.
nohup bash -c '
  rc=0
